...
describe('classifyRejection', () => {
  ...
  // ===== Red：生产崩溃复现 =====
  // 生产环境曾因飞书 230027（外部会话无权限）patch 失败
  // 触发 unhandledRejection 而退出。错误经 SDK throttle detach 路径逃逸到 handler，
  // handler 把 status=400 判为 fatal → process.exit(1)，正在进行的 claude run 陪葬。
