...
describe('JsonlRpcTransport safety and cleanup', () => {
  ...
  it('test_anchor_transport_forwards_full_env_including_provider_keys', async () => {
    // review P2-2 修正：app-server 与 exec 模式（getSpawnEnv 全量透传）共用
    // 同一 codex 二进制，provider 认证靠 OPENAI_API_KEY / 自定义 provider 的
    // env_key，代理环境靠 HTTP(S)_PROXY。此前收窄到 5 键白名单会打断认证与
    // 网络，因此必须全量透传 process.env。回归锚点：白名单收窄曾导致
    // provider env key 到不了子进程（认证失败）。
    ...
    // 普通 process.env 键同样全量透传（与 exec 模式对齐）
