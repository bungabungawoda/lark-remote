 ...
 * and timeout handling over a JsonlRpcTransport.
 */
...
export class CodexAppServerClient {
  ...
  constructor(transport: JsonlRpcTransport, hooks: ClientHooks, requestTimeoutMs: number = 60_000) {
   ...
   * leaving dead slots behind after the app-server process was killed.
   */
  ...
  async connect(): Promise<InitializeResult> {
    ...
    if (this.transport.closed) {
      throw new ConnectionLostError('codex app-server failed to start (binary missing or crashed)');
    }
    ...
    const result = await this.request<InitializeParams, InitializeResult>('initialize', initParams);
  ...
  private handleMessage(msg: object): void {
    ...
    if ('method' in message && 'id' in message) {
      // Server request (incoming request from server)
      const id = message.id as string | number;
      const method = message.method as string;
    ...
    getLogger().warn(
      `[codex-client] unknown message type: ${JSON.stringify(message).slice(0, 200)}`,
    );
  }
