/**
 * JSONL-RPC transport for the Codex app-server protocol.
 *
 * Wraps a child process (spawned from `codex app-server`) with
 * JSON-line-based bidirectional communication: each line on stdout is a JSON
 * message from the server; each `write()` sends a JSON line to stdin.
 */
...
export class JsonlRpcTransport {
  ...
  async start(events: TransportEvents): Promise<void> {
    this.events = events;
    // 与 exec 模式对齐（SpawningRunner.getSpawnEnv 全量透传 process.env）：
    // codex 是用户自己的可信二进制，provider 认证依赖 OPENAI_API_KEY /
    // ANTHROPIC_API_KEY / 自定义 provider 的 env_key（codex-config.ts
    // providerEnvKeys），代理环境依赖 HTTP(S)_PROXY、TMPDIR 等；任何白名单
    // 收窄都会打断认证或网络。调用方 this.env 覆盖 process.env 同名键。
    ...
    getLogger().info(`[codex-transport] spawned pid=${proc.pid} binary=${this.binary}`);
    proc.stdin?.on('error', (err) => {
      getLogger().warn(`[codex-transport] stdin error: ${err.message}`);
    ...
    proc.stderr?.on('data', (chunk: Buffer) => {
      ...
      getLogger().debug(`[codex-transport] stderr: ${text.trimEnd()}`);
    });
    ...
    proc.stdout?.on('data', (chunk: Buffer) => {
      ...
      for (const line of lines) {
        ...
        if (line.length > MAX_LINE_BYTES) {
          getLogger().error(
            `[codex-transport] line exceeds ${MAX_LINE_BYTES} bytes, disconnecting`,
          );
        ...
        try {
          ...
          getLogger().warn(
            `[codex-transport] failed to parse JSON: ${(err as Error).message} line=${line.slice(0, 200)}`,
          );
        }
      }
    });
    ...
    const onExit = (code: number | null, signal: string | null) => {
      ...
      events.onClose(reason);
      getLogger().info(
        `[codex-transport] process exited pid=${proc.pid} code=${code} signal=${signal}`,
      );
    };
    proc.once('exit', onExit);
    proc.once('error', (err) => {
      getLogger().error(`[codex-transport] process error: ${err.message}`);
  ...
  private handleClose(reason: string): void {
    ...
    if (proc) {
      ...
      void this.stopper.stop(proc).catch((err: unknown) => {
        getLogger().warn(
          `[codex-transport] failed to stop process during close: ${
    ...
    getLogger().info(`[codex-transport] closed: ${reason}`);
  }
}
