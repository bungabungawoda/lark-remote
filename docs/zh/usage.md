...
- **关键词搜索**：`/ls` 与 `/ws` 卡片上半区各有一行搜索输入框，输入关键词后点右侧 ✓ 提交（飞书 CardKit 的 input 没有逐键回调，只能确认制）。`/ws` 匹配别名或完整路径，`/ls` 只匹配**当前层**条目名（不递归子目录），均大小写不敏感、子串包含。筛选态下翻页/刷新/排序/使用/删除会保留关键词，进子目录/上级/返回/切换则清除筛选（换目录后旧关键词无意义）；清空输入框再点 ✓ 即取消筛选。计数与页码都按过滤后的结果计算。
