...
`/cd` path resolution must expand `~` first: `path.resolve` does not recognize `~`, and passing `~/projects` directly would be treated as a relative path, resulting in `<bridge process.cwd()>/~/projects`. `cmdCd` uses `path.join(os.homedir(), target.slice(1))` to preprocess `~`-prefixed input.
